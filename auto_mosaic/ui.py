...
from auto_mosaic.image_ops import load_image_bgr, visualize_detection
...
class AutoMosaicWindow(QMainWindow):
    ...
    def _build_layout(self) -> None:
        ...
        self.preview_mode_combo.currentTextChanged.connect(self._refresh_preview)
        ...
        self.effect_size_slider.valueChanged.connect(
            lambda value: self.effect_size_value.setText(f"{value} px")
        )
    ...
    def _on_selection_changed(self, row: int) -> None:
        if 0 <= row < len(self.image_paths):
            self.current_result = None
            self._show_original(self.image_paths[row])
            self.analysis_generation += 1
    ...
    def _process_current(self) -> None:
        ...
        def work() -> None:
            try:
                ...
                self.events.put(("single_complete", (result, output, remove_after)))
            except Exception as error:
    ...
    def _refresh_preview(self, _value: str | None = None) -> None:
        ...
        if mode == "元画像" or self.current_result is None:
            ...
            self._set_preview_bgr(
                visualize_detection(original, self.current_result.mask, self.current_result.detections)
            )
        else:
    ...
    def _set_busy(self, busy: bool, status: str) -> None:
        ...
        self.process_current_button.setEnabled(not busy)
        ...
        try:
            while True:
                ...
                if kind == "analysis":
                    ...
                    result, output, remove_after = payload  # type: ignore[misc]
